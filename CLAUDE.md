...
| `src/framework/components/reveal/Reveal.tsx` | Segment-based show/hide (AnimatePresence or keepInDOM) |
...
- Accessibility: animations respect `prefers-reduced-motion` via `useReducedMotion()` hook. **WSL2 gotcha**: browsers inherit the Windows host setting — if animations appear to not work, check Windows **Settings → Accessibility → Visual effects → Animation effects** is On. Diagnose with `window.matchMedia('(prefers-reduced-motion: reduce)').matches` in DevTools.
...
- **`<Reveal>` is the preferred pattern for segment-based visibility** — use `useSegmentedAnimation()` hook only for non-visual logic (data computation, conditional styling, components with built-in `isVisible` props). Note: `Reveal` uses `AnimatePresence` (mount/unmount) by default — this can cause layout shifts when content appears. Use `keepInDOM` to always render the element and animate between visible/hidden variants (avoids layout shifts). Use `animation={fadeIn}` to avoid Y-translate shifts, or wrap in `<RevealSequence>` for coordinated exit-before-enter swaps.
